...
static void exit_security(void) {
    if (g_security_ctx.audit_initialized)
        audit_exit();
    if (g_security_ctx.validation_initialized)
        validation_exit();
    if (g_security_ctx.crypto_initialized)
        crypto_exit();
}
...
int main(int argc, char* argv[]) {
    ...
    rc = init_security();
    if (R_FAILED(rc)) {
        printf("Failed to initialize security: 0x%x\n", rc);
        consoleUpdate(NULL);
    ...
    rc = app_init();
    if (R_SUCCEEDED(rc)) {
        // Run security tests in debug builds
#ifdef DEBUG
        TestSuite test_suite;
        test_suite_init(&test_suite);
        test_run_all(&test_suite);
        test_save_results(&test_suite, "sdmc:/dbfm/tests/security_tests.json");
        ...
        printf("Failed to initialize application: 0x%x\n", rc);
        consoleUpdate(NULL);
